				...
				foreach ($_gallery as $_image )
 				{
					...
					$product_base['image'][] = strval(Mage::helper('catalog/image')->init($_product, 'base', $_image->getFile())->resize(135));
				}
			}
		}
		return $product_base;	
		...
		foreach($_attributes as $_attribute):
			...
			if(in_array($_attribute['attribute_code'], $swatch_attributes)):
				...
				foreach($attributed->getSource()->getAllOptions(true, true) as $option){
 				...
 				$html .= '<div class="swatch-category-container" style="clear:both;">';
				...
				foreach($_attribute['values'] as $value):
					...
					if (count($matches[0]) > 0):
						...
						if (Mage::helper('colorselectorplus')->getSwatchUrl($theId)):
							...
							$html .= "'".$productId."','".$product_image."');";
							$html .= '" src="'.$swatchimage.'" id="'.$theId.'" class="swatch-category" alt="'.$altText.'" width="'.$width.'px" height="'.$height.'px" title="'.$altText.'" />';
						elseif($pos !== false):
							$html .= '<div onclick="listSwitcher(';
							$html .= "'".$productId."','".$product_image."');";
							$html .= '" id="'.$theId.'" class="swatch-category" style="background-color:'.$color_value.'; width:'.$width.'px; height:'.$height.'px;" title="'.$altText.'">';
							$html .= '</div>';
						else:
							$swatchimage = Mage::helper('colorselectorplus')->getSwatchUrl('empty');
							$html .= '<img src="'.$swatchimage.'" id="'.$theId.'" class="swatch-category" alt="'.$altText.'" width="'.$width.'px" height="'.$height.'px" title="'.$altText.'" />';
						endif;
