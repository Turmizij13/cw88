 ...
 */
class Dono_PageCache_Model_Processor_Product extends Dono_PageCache_Model_Processor_Default
{
    /**
    ...
    public function prepareContent(Zend_Controller_Response_Http $response)
    {
        ...
        if (!$cacheInstance->getFrontend()->test($cacheId)) {
            $cacheInstance->save($countLimit, $cacheId, array(Dono_PageCache_Model_Processor::CACHE_TAG));
        }
        // save current product id
        $product = Mage::registry('current_product');
        if ($product) {
            $cacheId = $processor->getRequestCacheId() . '_current_product_id';
            $cacheInstance->save($product->getId(), $cacheId, array(Dono_PageCache_Model_Processor::CACHE_TAG));
            $processor->setMetadata(self::METADATA_PRODUCT_ID, $product->getId());
            Dono_PageCache_Model_Cookie::registerViewedProducts($product->getId(), $countLimit);
        }
