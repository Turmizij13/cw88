<?php
class Dono_PageCache_Model_Cookie extends Mage_Core_Model_Cookie
{
    /**
     * Cookie names
     */
    ...
    protected function _getSalt()
    {
        if ($this->_salt === null) {
            ...
            $this->_salt = Dono_PageCache_Model_Cache::getCacheInstance()->load($saltCacheId);
            if (!$this->_salt) {
                $this->_salt = md5(microtime() . rand());
                Dono_PageCache_Model_Cache::getCacheInstance()->save($this->_salt, $saltCacheId,
    ...
    public static function registerViewedProducts($productIds, $countLimit, $append = true)
    {
        ...
        if ($append) {
            if (!empty($_COOKIE[Dono_PageCache_Model_Container_Viewedproducts::COOKIE_NAME])) {
                $cookieIds = $_COOKIE[Dono_PageCache_Model_Container_Viewedproducts::COOKIE_NAME];
        ...
        setcookie(Dono_PageCache_Model_Container_Viewedproducts::COOKIE_NAME, $cookieIds, 0, '/');
    }
