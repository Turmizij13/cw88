 ...
 */
class Dono_PageCache_Model_Container_CatalogProductItem extends Dono_PageCache_Model_Container_Advanced_Quote
{
     ...
     *
     * @var null| Dono_TargetRule_Block_Catalog_Product_List_Abstract
     */
    ...
    protected function _getListBlockType()
    {
        ...
        if ($blockName == self::BLOCK_NAME_RELATED) {
            return 'dono_targetrule/catalog_product_list_related';
        } elseif ($blockName == self::BLOCK_NAME_UPSELL) {
            return 'dono_targetrule/catalog_product_list_upsell';
        }
        ...
        if (is_null($this->_infoCacheId)) {
            $this->_infoCacheId = 'CATALOG_PRODUCT_LIST_SHARED_'
                . md5($this->_placeholder->getName()
                    . $this->_getCookieValue(Dono_PageCache_Model_Cookie::COOKIE_CART, '')
                    . $this->_getProductId());
        }
     ...
     *
     * @return Dono_PageCache_Model_Container_CatalogProductItem
     */
    protected function _saveInfoCache()
    {
        ...
        $data = array();
        $cacheRecord = Dono_PageCache_Model_Container_Abstract::_loadCache($this->_getCacheId());
        if ($cacheRecord) {
        ...
        $tags = array(Dono_PageCache_Model_Processor::CACHE_TAG);
        $lifetime = $this->_placeholder->getAttribute('cache_lifetime');
        if (!$lifetime) {
            $lifetime = false;
        }
        Dono_PageCache_Model_Cache::getCacheInstance()->save($data, $this->_getCacheId(), $tags, $lifetime);
        return $this;
    }
    ...
    protected function _getSharedParam($key = null)
    {
        ...
        if (is_null($info)) {
            $info = array();
            $cacheRecord = Dono_PageCache_Model_Cache::getCacheInstance()->load($this->_getCacheId());
            if ($cacheRecord) {
                $cacheRecord = json_decode($cacheRecord, true);
     ...
     *
     * @return false|Dono_TargetRule_Block_Catalog_Product_List_Abstract
     */
