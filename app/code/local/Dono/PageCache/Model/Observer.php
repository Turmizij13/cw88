<?php
class Dono_PageCache_Model_Observer
{
    /*
     ...
     *
     * @var Dono_PageCache_Model_Processor
     */
     ...
     * Page Cache Config
     *
     * @var Dono_PageCache_Model_Config
     */
    protected $_config;
    ...
    {
        $this->_processor = Mage::getSingleton('dono_pagecache/processor');
        $this->_config    = Mage::getSingleton('dono_pagecache/config');
     ...
     * @return Dono_PageCache_Model_Observer
     */
    ...
    public function processPreDispatch(Varien_Event_Observer $observer)
    {
        ...
        $noCache = $this->_getCookie()->get(Dono_PageCache_Model_Processor::NO_CACHE_COOKIE);
        if ($noCache) {
            Mage::getSingleton('catalog/session')->setParamsMemorizeDisabled(false);
            $this->_getCookie()->renew(Dono_PageCache_Model_Processor::NO_CACHE_COOKIE);
        } elseif ($action) {
    ...
    protected function _saveDesignException()
    {
        ...
        $cacheId = Dono_PageCache_Model_Processor::DESIGN_EXCEPTION_KEY;

        $exception = Dono_PageCache_Model_Cache::getCacheInstance()->load($cacheId);
        if (!$exception) {
            $exception = Mage::getStoreConfig(self::XML_PATH_DESIGN_EXCEPTION);
            Dono_PageCache_Model_Cache::getCacheInstance()->save($exception, $cacheId);
    ...
    public function validateDataChanges(Varien_Event_Observer $observer)
    {
        ...
        $object = $observer->getEvent()->getObject();
        Mage::getModel('dono_pagecache/validator')->checkDataChange($object);
        return $this;
    }
    ...
    public function validateDataDelete(Varien_Event_Observer $observer)
    {
        ...
        $object = $observer->getEvent()->getObject();
        Mage::getModel('dono_pagecache/validator')->checkDataDelete($object);
        return $this;
    }
    ...
    {
        Dono_PageCache_Model_Cache::getCacheInstance()->clean(Dono_PageCache_Model_Processor::CACHE_TAG);
        return $this;
    }
    ...
    {
        Dono_PageCache_Model_Cache::getCacheInstance()->getFrontend()->clean(Zend_Cache::CLEANING_MODE_OLD);
        return $this;
    }
    ...
    public function registerQuoteChange(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->setObscure(Dono_PageCache_Model_Cookie::COOKIE_CART, 'quote_' . $quote->getId());

        $cacheId = Dono_PageCache_Model_Container_Advanced_Quote::getCacheId();
        Dono_PageCache_Model_Cache::getCacheInstance()->remove($cacheId);
    ...
    public function registerCompareListChange(Varien_Event_Observer $observer)
    {
        ...
        $previouseList = $this->_getCookie()->get(Dono_PageCache_Model_Cookie::COOKIE_COMPARE_LIST);
        ...
        sort($ids);
        $this->_getCookie()->set(Dono_PageCache_Model_Cookie::COOKIE_COMPARE_LIST, implode(',', $ids));

        //Recenlty compared products processing
        $recentlyComparedProducts = $this->_getCookie()
            ->get(Dono_PageCache_Model_Cookie::COOKIE_RECENTLY_COMPARED);
        ...
        $this->_getCookie()->set(Dono_PageCache_Model_Cookie::COOKIE_RECENTLY_COMPARED,
    ...
    public function processNewMessage(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->set(Dono_PageCache_Model_Cookie::COOKIE_MESSAGE, '1');
        return $this;
    }
    ...
    {
        try {
            $productIds = $this->_getCookie()->get(Dono_PageCache_Model_Container_Viewedproducts::COOKIE_NAME);
            if ($productIds) {
    ...
    public function customerLogout(Varien_Event_Observer $observer)
    {
        ...
        if (!$this->_getCookie()->get(Dono_PageCache_Model_Cookie::COOKIE_CUSTOMER)) {
            $this->_getCookie()->delete(Dono_PageCache_Model_Cookie::COOKIE_RECENTLY_COMPARED);
            $this->_getCookie()->delete(Dono_PageCache_Model_Cookie::COOKIE_COMPARE_LIST);
            Dono_PageCache_Model_Cookie::registerViewedProducts(array(), 0, false);
        }
    ...
    public function registerWishlistChange(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->setObscure(Dono_PageCache_Model_Cookie::COOKIE_WISHLIST, $cookieValue);

        // Wishlist items count hash for top link
        $this->_getCookie()->setObscure(Dono_PageCache_Model_Cookie::COOKIE_WISHLIST_ITEMS,
    ...
    public function registerWishlistListChange(Varien_Event_Observer $observer)
    {
        ...
        $blockContainer = Mage::getModel('dono_pagecache/container_wishlists');
        Dono_PageCache_Model_Cache::getCacheInstance()->remove($blockContainer->getCacheId());
    ...
    public function registerPollChange(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->set(Dono_PageCache_Model_Cookie::COOKIE_POLL, $cookieValue);
    ...
    public function registerNewOrder(Varien_Event_Observer $observer)
    {
        ...
        $cacheId = md5($this->_getCookie()->get(Dono_PageCache_Model_Cookie::COOKIE_CUSTOMER));
        Dono_PageCache_Model_Cache::getCacheInstance()->remove($cacheId);
        return $this;
    }
    ...
    public function processMessageClearing(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->delete(Dono_PageCache_Model_Cookie::COOKIE_MESSAGE);
        return $this;
    }
    ...
    public function registerDesignExceptionsChange(Varien_Event_Observer $observer)
    {
        ...
        Dono_PageCache_Model_Cache::getCacheInstance()
            ->save($object->getValue(), Dono_PageCache_Model_Processor::DESIGN_EXCEPTION_KEY,
                array(Dono_PageCache_Model_Processor::CACHE_TAG));
        return $this;
    }
     ...
     *
     * @return Dono_PageCache_Model_Cookie
     */
    protected function _getCookie()
    {
        return Mage::getSingleton('dono_pagecache/cookie');
    }
    ...
    {
        $varName = Dono_PageCache_Model_Processor::LAST_PRODUCT_COOKIE;
    ...
    public function updateProductInfo(Varien_Event_Observer $observer)
    {
        ...
        if ($paramsObject instanceof Varien_Object) {
            if (array_key_exists(Dono_PageCache_Model_Cookie::COOKIE_CATEGORY_ID, $_COOKIE)) {
                $paramsObject->setCategoryId($_COOKIE[Dono_PageCache_Model_Cookie::COOKIE_CATEGORY_ID]);
            }
        }
        return $this;
    }
    ...
    public function checkMessages(Varien_Event_Observer $observer)
    {
        ...
        if ($httpHost != $urlHost && Mage::getSingleton('core/session')->getMessages()->count() > 0) {
            $transport->setUrl(Mage::helper('core/url')->addRequestParam(
                $url,
                array(Dono_PageCache_Model_Cache::REQUEST_MESSAGE_GET_PARAM => null)
            ));
        }
        return $this;
    }
    ...
    public function changedCustomerSegmentIds(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->set(Dono_PageCache_Model_Cookie::CUSTOMER_SEGMENT_IDS, $segmentsIdsString);
    }
}
